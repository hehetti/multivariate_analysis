...
def main():
    # 1. 원본 데이터 로드
    input_path = "../data/청주_열공급량_기상변수.csv"
    output_path = "../data/final_dataset_complete.csv" # 최종 저장 파일명
    ...
    # 2. 파이프라인 실행
    df = add_cyclic_time_features(df)         # 시간 변수 (먼저 해야 month, weekday 사용 가능)
    df = add_weather_features(df)             # 기상 변수 (heat_stress 등)
    df = add_calendar_features(df)            # 달력 변수 (코로나, 공휴일, 강수량)

    # 3. Lag 생성 (대상 컬럼 지정)
    target_cols_for_lag = [
        '기온(°C)', '상대습도(%)', '풍속(m/s)',
        'heat_stress', 'cold_stress', 'wind_x', 'wind_y'
    ]
